...
use nrr_provider::cran::{CranCatalog, DcfDocument};
...
fn package_name_rule_rejects_hyphens_and_trailing_periods() {
    assert!(PackageName::new("nrrfixture-core").is_err());
    assert!(PackageName::new("nrrfixture.").is_err());
}

#[test]
fn catalog_rejects_malformed_dependency_entry_through_provider_parser() {
    let malformed = b"Package: nrrfixture.invalid\nVersion: 1.0.0\nDepends: nrrfixture.valid (>= 1.0) trailing\n";
