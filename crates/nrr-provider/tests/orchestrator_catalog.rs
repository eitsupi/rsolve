...
use nrr_core::*;
use nrr_provider::cran::CranCatalog;
...
fn r_survives_as_a_real_requirement() {
    let c = catalog();
    let core = only(&c, "nrrfixture.core");
    let r = core
        .dependencies()
        .iter()
...
fn folded_dependency_list_is_split_into_entries() {
    ...
    assert_eq!(
        imports,
        vec!["nrrfixture.import", "nrrfixture.import.helper"],
...
fn unconstrained_dependency_accepts_any_version() {
    ...
    let base = core
        .dependencies()
        .iter()
        .find(|d| d.name.as_str() == "nrrfixture.base")
...
fn a_malformed_record_is_reported_not_silently_dropped() {
    ...
    assert!(
        !c.packages()
            .any(|(name, _)| name.as_str() == "nrrfixture.broken"),
