use nrr_core::{DependencyKind, DependencySourceConstraint, Provenance, RelationOp};
use nrr_provider::cran::{CranCatalog, CranRecordError, DependencyParseError};
...
fn folded_dependencies_and_absent_optional_fields_are_handled() {
    ...
    let folded = catalog.candidates_named("nrrfixture.folded").unwrap();
    ...
    assert_eq!(
        ...
        "nrrfixture.suggest.24"
    );

    let plain = &catalog.candidates_named("nrrfixture.plain").unwrap()[0];
