use nrr_provider::cran::{CranArchiveIndexError, CranCatalog, CranRecordError};
...
fn archive_rows_are_not_assumed_to_be_version_ordered() {
    ...
    let history = catalog.candidates_named("nrrfixture.history").unwrap();
    assert_eq!(
        history
            .iter()
    ...
    assert_eq!(
        ...
        Some(&"NRR Fictional Terms Older".to_owned())
    );
    assert_eq!(
        history[1].metadata().fields().get("License"),
        Some(&"NRR Fictional Terms – Archive".to_owned())
    );
}
...
fn archive_reader_looks_up_columns_by_name_and_preserves_dependencies() {
    ...
    assert_eq!(
        ...
        Some(Some("nrrfixture.history"))
    );

    let catalog = CranCatalog::from_archive_index_rds(ARCHIVE).expect("archive fixture");
    let release = &catalog.candidates_named("nrrfixture.history").unwrap()[1];

    for (kind, name) in [
        (nrr_core::DependencyKind::Depends, "R"),
        (nrr_core::DependencyKind::Imports, "nrrfixture.import.extra"),
        (nrr_core::DependencyKind::LinkingTo, "nrrfixture.link"),
        (nrr_core::DependencyKind::Suggests, "nrrfixture.suggest"),
        (nrr_core::DependencyKind::Enhances, "nrrfixture.enhance"),
    ] {
        assert!(
            release
                .dependencies()
    ...
    assert!(
        release
            .dependencies()
            .iter()
            .any(|dependency| dependency.name.as_str() == "nrrfixture.import.helper")
    );
    assert_eq!(
        catalog.candidates_named("nrrfixture.utf8").unwrap()[0]
            .metadata()
            .fields()
            .get("License"),
        Some(&"NRR Fictional Terms 日本語".to_owned())
    );
}
...
fn malformed_archive_row_is_skipped_with_source_index_and_package() {
    ...
    assert_eq!(diagnostic.package(), Some("nrrfixture.broken"));
    assert!(matches!(
        diagnostic.error(),
    ...
    assert!(
        catalog
            .candidates_named("nrrfixture.broken")
            .unwrap()
            .is_empty()
    );
}
