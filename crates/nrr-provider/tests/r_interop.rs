...
use nrr_provider::cran::DcfDocument;

fn external_packages_path() -> PathBuf {
    let root = env::var_os("NRR_CRAN_CORPUS")
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| {
            panic!(
                "NRR_CRAN_CORPUS is required; provide a repository root containing src/contrib/PACKAGES"
            )
        });
    let root = PathBuf::from(root);
    assert!(
        root.is_dir(),
        "NRR_CRAN_CORPUS is not a directory: {}",
        root.display()
    );
