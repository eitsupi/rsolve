...
fn packages_reject_multiple_version_clauses_for_one_dependency() {
    ...
    let middle = MaterializationArtifact::new(
        ReleaseIdentity::new(
            ...
            Provenance::ImmutableSource {
                scheme: rsolve_core::SourceScheme::new("fixture").unwrap(),
                digest: rsolve_core::Sha256Digest::new("b".repeat(64)).unwrap(),
            },
        ),
        RPackageVersion::parse("0.1.0").unwrap(),
        CachedArtifact {
            ...
            sha256: rsolve_core::Sha256Digest::new("b".repeat(64)).unwrap(),
            size: 1,
            verification: VerificationStrength::None,
        },
    )
...
fn canonicalizes_set_order_for_state_and_reruns() {
    ...
    assert_eq!(
        fs::read(first_project.join(".rsolve/materialization.toml")).unwrap(),
        fs::read(second_project.join(".rsolve/materialization.toml")).unwrap()
    );
...
fn recovers_repository_published_before_state_rename() {
    ...
    let rsolve = project_root.join(".rsolve");
    let state_path = rsolve.join("materialization.toml");
    let state_temp_name = ".materialization.toml.partial.crash";
    let state_temp = rsolve.join(state_temp_name);
    ...
    fs::write(
        rsolve.join(".materialization.transaction.toml"),
    ...
    assert!(!rsolve.join(".materialization.transaction.toml").exists());
    fs::remove_dir_all(cache_root).unwrap();
    fs::remove_dir_all(project_root).unwrap();
}
...
fn recovers_marker_before_repository_rename() {
    ...
    let rsolve = project_root.join(".rsolve");
    let repository = rsolve.join("repository");
    let staging_name = ".repository.partial.crash";
    let staging = rsolve.join(staging_name);
    let state_path = rsolve.join("materialization.toml");
    let state_temp_name = ".materialization.toml.partial.crash";
    let state_temp = rsolve.join(state_temp_name);
    let state_bytes = fs::read(&state_path).unwrap();
...
fn recovers_valid_pre_marker_orphan_without_transaction_temp() {
    ...
    let state_temp = rsolve.join(state_temp_name);
    let transaction_temp = rsolve.join(".materialization.transaction.toml.partial.orphan");
    let state_bytes = fs::read(&state_path).unwrap();
    fs::rename(&repository, &staging).unwrap();
    fs::rename(&state_path, &state_temp).unwrap();
    ...
    assert!(!staging.exists());
    assert!(rsolve.join("repository").is_dir());
    fs::remove_dir_all(cache_root).unwrap();
    fs::remove_dir_all(project_root).unwrap();
}
...
fn does_not_remove_symlink_or_ambiguous_pre_marker_temps() {
    ...
    let rsolve = project_root.join(".rsolve");
    fs::create_dir_all(&rsolve).unwrap();
    let target = temporary_root("materialize-orphan-symlink-target");
    let state_link = rsolve.join(".materialization.toml.partial.symlink");
    ...
    fs::write(rsolve.join(".materialization.toml.partial.a"), b"").unwrap();
    fs::write(rsolve.join(".materialization.toml.partial.b"), b"").unwrap();
    assert!(matches!(
        materialize(MaterializationRequest::new(
            &project_root,
            std::slice::from_ref(&selected),
    ...
    assert!(rsolve.join(".materialization.toml.partial.a").exists());
    assert!(rsolve.join(".materialization.toml.partial.b").exists());
    fs::remove_dir_all(cache_root).unwrap();
    fs::remove_dir_all(project_root).unwrap();
}
...
fn materializes_with_local_gitignore_state_and_independent_bytes() {
    ...
    .unwrap();
    assert_eq!(
        fs::read(project_root.join(".rsolve/.gitignore")).unwrap(),
        b"*\n"
    );
    ...
    let destination = project_root.join(".rsolve/repository/src/contrib/example_1.2-3.tar.gz");
    assert_eq!(fs::read(&destination).unwrap(), bytes);
    let packages = project_root.join(".rsolve/repository/src/contrib/PACKAGES");
...
fn materialization_rejects_duplicate_packages_and_existing_repository() {
    ...
    fs::create_dir_all(project_root.join(".rsolve/repository")).unwrap();
    assert!(matches!(
        materialize(MaterializationRequest::new(
            &project_root,
...
fn interrupted_transaction_rejects_changed_packages_metadata_before_publish() {
    ...
    let rsolve = project_root.join(".rsolve");
    let repository = rsolve.join("repository");
    let staging = rsolve.join(".repository.partial.index-mismatch");
    let state_path = rsolve.join("materialization.toml");
    let state_temp = rsolve.join(".materialization.toml.partial.index-mismatch");
    let state_bytes = fs::read(&state_path).unwrap();
    fs::rename(&repository, &staging).unwrap();
    fs::rename(&state_path, &state_temp).unwrap();
    let mut marker: toml::Value =
...
fn invalid_packages_metadata_leaves_no_staging_and_retry_succeeds() {
    ...
    let rsolve = project_root.join(".rsolve");
    assert!(!rsolve.join("repository").exists());
    assert!(!rsolve.join("materialization.toml").exists());
    assert!(!rsolve.join(".materialization.transaction.toml").exists());
    assert_eq!(
        fs::read_dir(&rsolve)
            .unwrap()
