...
pub(super) fn recover(
    ...
    let states = partial_paths(rsolve, &format!(".{STATE_NAME}.partial."))?;
    let markers = partial_paths(rsolve, &format!("{TRANSACTION_NAME}.partial."))?;
    let stagings = partial_paths(rsolve, STAGING_PREFIX)?;
    if !fs::symlink_metadata(transaction_path).is_ok() {
        if reconstruct(rsolve, &states, &markers, &stagings, requested)? {
            return recover(rsolve, state_path, transaction_path, repository, requested);
        }
        return Ok(());
    }
    ...
    let state_temp = transaction_temp_path(rsolve, &tx.state_temp)
        .ok_or_else(|| transaction_conflict(transaction_path, "invalid state temporary path"))?;
    let staging = staging_path(rsolve, &tx.staging_dir)
    ...
    if repo_exists {
        ...
        sync_directory(rsolve).map_err(|source| MaterializationError::Cache {
            path: rsolve.to_path_buf(),
            source,
        })?;
    }
...
fn reconstruct(
    ...
    validate_index_digest(&tx, requested)?;
    validate_repository_contents(staging, &tx.records, &tx.index_sha256)?;
    let marker = rsolve.join(TRANSACTION_NAME);
    if let Some(path) = markers.first() {
        ...
        let temp = rsolve.join(format!("{TRANSACTION_NAME}.partial.{}", unique_nonce()));
...
fn transaction_temp_path(rsolve: &Path, name: &str) -> Option<PathBuf> {
    safe_temp_name(name, &format!(".{STATE_NAME}.partial.")).then(|| rsolve.join(name))
}

fn staging_path(rsolve: &Path, name: &str) -> Option<PathBuf> {
    safe_temp_name(name, STAGING_PREFIX).then(|| rsolve.join(name))
}
