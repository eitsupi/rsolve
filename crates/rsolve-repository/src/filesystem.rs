...
pub(super) fn ensure_gitignore(rsolve: &Path) -> Result<(), MaterializationError> {
    let path = rsolve.join(GITIGNORE_NAME);
    if path.exists() {
        ...
        let existing =
            fs::read(&path).map_err(|source| io_error("read .rsolve/.gitignore", &path, source))?;
    ...
    fs::write(&path, GITIGNORE)
        .map_err(|source| io_error("create .rsolve/.gitignore", &path, source))
}
