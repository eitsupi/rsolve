...
fn run_r(repository_url: &str, library: &Path, script: &str) {
    ...
    let rscript = env::var_os("RSOLVE_RSCRIPT").unwrap_or_else(|| "Rscript".into());
    let output = Command::new(rscript)
        .arg("--vanilla")
        .arg(&script_path)
        .env("RSOLVE_REPOSITORY", repository_url)
        .env("RSOLVE_LIBRARY", library)
        .output()
    ...
    eprintln!(
        "RSOLVE_R_INTEROP status=ok stdout={}",
        String::from_utf8_lossy(&output.stdout)
    );
}
...
fn repository_round_trips_through_real_r() {
    ...
    let contrib = root.join(".rsolve/repository/src/contrib");
    ...
    let r_script = r#"
repo <- Sys.getenv("RSOLVE_REPOSITORY")
lib <- Sys.getenv("RSOLVE_LIBRARY")
...
cat("RSOLVE_R_INTEROP available_packages=ok install_order=ok wrong_order=failed\n")
"#;
    let repository_url = Url::from_directory_path(root.join(".rsolve/repository"))
        .unwrap()
        .to_string();
