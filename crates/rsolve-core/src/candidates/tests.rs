...
use crate::{
    ...
    ReleaseObservation, SourceArtifact,
};
...
fn merge_is_identity_keyed_and_occurrence_order_is_canonical() {
    let first =
        PreparedCandidate::new(release("foo", "1.0.0"), false, vec![occurrence("two", 1)]).unwrap();
    let second =
        PreparedCandidate::new(release("foo", "1.0.0"), false, vec![occurrence("one", 0)]).unwrap();
...
fn direct_candidate_can_be_eligible_without_occurrence() {
    let candidate = PreparedCandidate::new(release("foo", "1.0.0"), true, Vec::new()).unwrap();
...
fn candidate_set_preserves_distinct_identities_and_merges_equal_ones() {
    let first =
        PreparedCandidate::new(release("foo", "1.0.0"), false, vec![occurrence("one", 0)]).unwrap();
    let second_identity =
        PreparedCandidate::new(release("foo", "2.0.0"), false, vec![occurrence("two", 1)]).unwrap();
    let same_identity =
        PreparedCandidate::new(release("foo", "1.0.0"), false, vec![occurrence("two", 1)]).unwrap();
    let set =
...
fn candidate_set_reports_same_identity_metadata_conflicts() {
    let first = PreparedCandidate::new(git_release("foo", "1.0.0"), true, Vec::new()).unwrap();
    let second = PreparedCandidate::new(git_release("foo", "2.0.0"), true, Vec::new()).unwrap();
    assert!(matches!(
...
fn candidate_set_keeps_existing_candidate_when_merge_fails() {
    let first = PreparedCandidate::new(git_release("foo", "1.0.0"), true, Vec::new()).unwrap();
    let conflicting =
        PreparedCandidate::new(git_release("foo", "2.0.0"), true, Vec::new()).unwrap();
