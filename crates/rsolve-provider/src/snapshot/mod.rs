...
mod tests {
    ...
    fn stored_header(path: &Path) -> Vec<u8> {
        let database = ReadOnlyDatabase::open(path).unwrap();
        let read = database.begin_read().unwrap();
    ...
    fn store_publishes_strict_pointer_and_pins_readers() {
        ...
        assert_ne!(old_reader.header.generation, new_reader.header.generation);
    ...
    fn refresh_lock_contention_is_visible_to_a_child_process() {
        ...
        let status = Command::new(std::env::current_exe().unwrap())
            .arg("--exact")
            .arg("snapshot::tests::refresh_lock_child_probe")
            .arg("--nocapture")
    ...
    fn read_current_serializes_generation_open_with_cleanup() {
        ...
        let mut child = Command::new(std::env::current_exe().unwrap())
            .arg("--exact")
            .arg("snapshot::tests::read_current_lock_child_probe")
            .arg("--nocapture")
    ...
    fn cleanup_preserves_generation_symlinks() {
        ...
        let link = store.root().join(
            "generations/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.redb",
        );
    ...
    fn cleanup_removes_unclean_child_generation_temp() {
        ...
        let status = Command::new(std::env::current_exe().unwrap())
            .arg("--exact")
            .arg("snapshot::tests::unclean_generation_child_probe")
            .arg("--nocapture")
    ...
    fn store_rejects_broken_pointer_and_cleans_orphans_without_touching_current() {
        ...
        let orphan_generation = store.root().join(
            "generations/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.redb",
        );
