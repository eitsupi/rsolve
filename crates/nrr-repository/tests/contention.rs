...
use nrr_core::{ArtifactLocator, SourceArtifact};
use nrr_repository::commit_source_artifact;
...
fn contention_parent() {
    if std::env::var_os("NRR_CONTENTION_CHILD").is_some() {
        return;
    }
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("nrr-contention-{nonce}"));
    ...
    for id in 0..2 {
        children.push(
            Command::new(&executable)
                ...
                .arg("--nocapture")
                .env("NRR_CONTENTION_CHILD", "1")
                .env("NRR_CONTENTION_ROOT", &root)
                .env("NRR_CONTENTION_GATE", &gate)
                .env(
                    "NRR_CONTENTION_READY",
...
fn contention_child() {
    let (Some(root), Some(gate), Some(ready)) = (
        std::env::var_os("NRR_CONTENTION_ROOT"),
        std::env::var_os("NRR_CONTENTION_GATE"),
        std::env::var_os("NRR_CONTENTION_READY"),
    ) else {
        return;
    };
