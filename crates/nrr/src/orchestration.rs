...
// intentionally absent; without runtime inventory, nrr must not infer that a
    ...
    fn new(loader: L, r_version: nrr_core::RPackageVersion) -> Result<Self, CandidateLoadError> {
        ...
        for name in R_BASE_PACKAGE_NAMES {
            ...
            let release = PackageRelease::try_from(ReleaseObservation {
                identity: nrr_core::ReleaseIdentity::new(
                    package.clone(),
...
/// `nrr-resolver`, where normal mode uses `PreferLocked` and frozen mode uses
/// `RequireLocked`.
...
fn resolve_request_with_mode(
    ...
    let lock_policy: &dyn nrr_resolver::LockUpdatePolicy = match mode {
