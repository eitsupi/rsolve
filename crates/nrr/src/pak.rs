...
//!
//! nrr passes an already-selected closure to one pak request. Package builds,
...
project <- normalizePath(Sys.getenv("NRR_PAK_PROJECT_LIBRARY"), mustWork = FALSE)
pak_library <- normalizePath(Sys.getenv("NRR_PAK_LIBRARY"), mustWork = FALSE)
...
metadata <- data.frame(Package = "__nrr_protocol__", Version = "1", Imports = "", LinkingTo = "", Description = "", Path = "", RemoteRepos = "", RemotePkgRef = "", RemoteSha = "", Status = "", Repository = repository_url, PakStatus = status, PakCalls = "1", stringsAsFactors = FALSE)
...
pub fn run_pak(
    ...
    command
        ...
        .env("NRR_PAK_PROJECT_LIBRARY", &config.project_library)
        .env("NRR_PAK_LIBRARY", &config.pak_library)
    ...
    let Some(protocol) = records
        .iter()
        .find(|record| record.get("Package").map(String::as_str) == Some("__nrr_protocol__"))
    else {
        return Err(PakProcessError::Protocol(
    ...
    let installed = records
        .iter()
        .filter(|record| record.get("Package").map(String::as_str) != Some("__nrr_protocol__"))
        .map(|record| {
