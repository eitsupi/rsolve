...
use rsolve::{PakArtifact, PakInstallRequest, PakProcessConfig, run_pak};
...
fn fresh_root(label: &str) -> PathBuf {
    ...
    let root = env::temp_dir().join(format!(
        "rsolve-pak-portable-{label}-{}-{nonce}",
        std::process::id()
    ));
...
fn config(root: &Path, repository: &Path, project: &Path) -> PakProcessConfig {
    PakProcessConfig {
        rscript: required_absolute("RSOLVE_RSCRIPT"),
        pak_library: required_absolute("RSOLVE_PAK_LIBRARY"),
        pak_private_library: required_absolute("RSOLVE_PAK_PRIVATE_LIBRARY"),
...
fn wrong_order_install(config: &PakProcessConfig) -> std::process::Output {
    ...
    command
        .args([
            ...
            "-e",
            r#"
repo <- Sys.getenv("RSOLVE_NEGATIVE_REPOSITORY")
lib <- Sys.getenv("RSOLVE_NEGATIVE_LIBRARY")
pak_library <- Sys.getenv("RSOLVE_NEGATIVE_PAK_LIBRARY")
private_library <- Sys.getenv("RSOLVE_NEGATIVE_PRIVATE_LIBRARY")
dir.create(lib, recursive = TRUE, showWarnings = FALSE)
.libPaths(c(lib, pak_library, private_library, .Library))
options(repos = c(rsolve = repo))
...
"#,
        ...
        .env("R_PROFILE_USER", "")
        .env("R_ENVIRON_USER", "")
        .env("RSOLVE_NEGATIVE_REPOSITORY", repository_url)
        .env("RSOLVE_NEGATIVE_LIBRARY", &config.project_library)
        .env("RSOLVE_NEGATIVE_PAK_LIBRARY", &config.pak_library)
        .env(
            "RSOLVE_NEGATIVE_PRIVATE_LIBRARY",
            &config.pak_private_library,
        );
...
fn assert_installed(result: &rsolve::PakInstallResult, project: &Path, expected: &[&str]) {
...
pub fn run_contract(expected_mode: &str, fixture: &Path) {
    assert_eq!(env::var("RSOLVE_TEST_MODE").as_deref(), Ok(expected_mode));
    ...
    let leaked_staging = fs::read_dir(correct_root.join("tmp"))
        ...
        .any(|entry| {
            entry
                ...
                .file_name()
                .to_string_lossy()
                .starts_with(".rsolve-pak-staging-")
        });
