...
use rsolve_core::{
    EnvironmentId, PackageName, Provenance, PublicationDate, RPackageVersion, ReleaseIdentity,
    RepositorySubdir, Sha256Digest, SourceScheme,
};

use crate::lock::{LockError, LockedPackage, LockedResolution, Lockfile};
...
fn decode_package(wire: WirePackage) -> Result<LockedPackage, LockWireError> {
    ...
    let mut dependencies = wire
        .dependencies
        .into_iter()
        .map(|value| {
            PackageName::new(value)
                .map_err(|error| invalid_field("package.dependencies", error.to_string()))
        })
        .collect::<Result<Vec<_>, _>>()?;
    dependencies.sort();
    dependencies.dedup();
...
fn serialize_wire(wire: &WireLockfile) -> Result<String, LockWireError> {
    ...
    for package in &wire.packages {
        ...
        output.push_str(&format!(
            "dependencies = {}\n",
            toml_array(&package.dependencies)?
        ));
    }
    Ok(output)
}
