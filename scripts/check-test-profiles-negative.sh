...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/rsolve-profile-negative.XXXXXX")
trap 'rm -rf "$tmp_dir"' EXIT
jq '."rust-suites" |= with_entries(select(.value["package-name"] == "rsolve-provider"))' \
...
assert_rejected() {
    ...
    default_fixture=$2
    profile_fixture=$3
    if sh "$evaluator" "$metadata" "$default_fixture" "$profile_fixture" rsolve-provider r_interop >/dev/null 2>&1; then
...
for field in package-id binary-id kind; do
    jq --arg field "$field" 'del(."rust-suites"["rsolve-provider::r_interop"][$field])' \
    ...
    jq --arg field "$field" '."rust-suites"["rsolve-provider::r_interop"][$field] = null' \
...
jq '."rust-suites" |= with_entries(select(.value["package-name"] == "rsolve-repository"))' \
...
assert_repository_rejected() {
    ...
    default_fixture=$2
    profile_fixture=$3
    if sh "$evaluator" "$metadata" "$default_fixture" "$profile_fixture" rsolve-repository repository_r_interop >/dev/null 2>&1; then
        echo "repository profile mutation unexpectedly passed: $label" >&2
        exit 1
    fi
    echo "repository profile mutation rejected: $label"
}
jq '."rust-suites"["rsolve-repository::repository_r_interop"].status = "listed"' \
...
jq '."rust-suites"["rsolve-repository::repository_r_interop"]["binary-id"] = "rsolve-repository::changed"' \
...
jq '."rust-suites"["rsolve-provider::r_interop"].status = "listed"' \
...
jq '."rust-suites"["rsolve-provider::r_interop"].status = "skipped-filter"' \
...
jq '."rust-suites"["rsolve-provider::r_interop"]["package-id"] = "path+file:///workspace/crates/changed#0.1.0"' \
...
jq '."rust-suites"["rsolve-provider::r_interop"]["binary-id"] = "rsolve-provider::changed"' \
...
jq '."rust-suites"["rsolve-provider::r_interop"].kind = "lib"' \
...
for kind in test lib bin example; do
    jq --arg kind "$kind" '
        .["rust-suites"]["rsolve-provider::fresh-\($kind)"] = {
            "package-name": "rsolve-provider",
            "binary-id": "rsolve-provider::fresh-\($kind)",
            "binary-name": "fresh-\($kind)",
            "package-id": "path+file:///workspace/crates/rsolve-provider#0.1.0",
            "kind": $kind,
            "status": "listed"
        }
